...
#pragma warning restore 0649
        ...
        public static DDSImage Load(Stream Source)
        {
            ...
            using (BinaryReader _data = new BinaryReader(Source))
            {
                ...
                {
                    throw new Exception("DDSImage.Load() requires a .dds texture file stream");
                }
                ...
                if ((_dds.PixelFormat.dwFlags & DDPF_FOURCC) != 0)
                {
                    switch (_dds.PixelFormat.dwFourCC)
                    {
                        case FOURCC_DX10:
                            ...
                            throw new Exception("DX10 textures not supported at this time.");
                        case FOURCC_DXT1:
                ...
                for (int _level = 0; _level < _dds.MipMapCount; _level++)
                {
                    ...
                    catch
                    {
                        ...
                        if (_level == 0)
                        {
                            ...
                            throw new Exception("Unable to read pixel data.");
                        }
                        else
                        {
        ...
        private class Decompress
        {
            public static Bitmap Image(byte[] Data, int W, int H, uint CompressionMode)
            {
                ...
                // https://msdn.microsoft.com/en-us/library/bb147243(v=vs.85).aspx
                ...
                if (_pixels == null)
                {
                    throw new Exception(string.Format("DDS compression Mode '{0}{0}{0}{0}' not supported.",
