...
namespace Taiko5DXSaveEditor.TableManagement
{
    ...
    /// 武将データテーブルの管理クラス
    /// </summary>
    public class BushoTableManager : GameDataTableManager
    {
        #region プロパティ
        /// <summary>
        /// テーブルの種類
        /// </summary>
        public override TableType TableType { get { return TableType.Busho; } }

        #endregion

        #region コンストラクタ
        /// <summary>
        /// 武将データテーブルの管理クラスのコンストラクタ
        /// </summary>
        /// <param name="gameDataTable">管理対象のテーブル</param>
        /// <param name="gameData">表示対象のゲームデータ</param>
        public BushoTableManager(DataGridView gameDataTable, GameData gameData)
        ...
        public override void InitializeTable()
        {
            ...
            _GameDataTable.Columns.Add("Kani", @"官位官職");
        ...
        public override void UpdateTable(IEnumerable<DataGridViewRow> selectedRows)
        {
            foreach (DataGridViewRow row in selectedRows)
            {
                ...
                var kani = GameDataTableCellValue.Empty;
                var yearOfDeath = GameDataTableCellValue.Empty;
                var spouse = GameDataTableCellValue.Empty;
                if (busho.PeopleCategory == PeopleCategory.Busho)
                {
                    if (busho.Kani != GameData.NoneKaniID)
                    {
                        kani.Text = _GameData.NameListDictionary["Kan-i"][busho.Kani];
                        kani.SortValue = busho.Kani;
                    }
                    yearOfDeath.Text = (busho.YearOfDeath + 1500).ToString();
                    yearOfDeath.SortValue = busho.YearOfDeath + 1500;
                    if (busho.Spouse != GameData.NoneBushoID)
                    {
                        spouse.Text = _GameData.BushoList[busho.Spouse].Name;
                        spouse.SortValue = busho.Spouse;
                    }
                }
                // 代入
